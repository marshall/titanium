...
static CURLcode custom_do(struct connectdata *, bool *done);
static CURLcode custom_done(struct connectdata *conn,
                          CURLcode status, bool premature);
static CURLcode custom_connect(struct connectdata *conn, bool *done);
...
static CURLcode custom_connect(struct connectdata *conn, bool *done)
{
  ...
  char *real_path = curl_easy_unescape(data, data->state.path, 0, NULL);
	char *host = curl_easy_unescape(data, conn->host.rawalloc, 0, NULL);
  ...
  char *new_path;
	int new_path_len = 0;
	struct Curl_local_handler* local_handler = get_local_handler(conn->protostr);

  if(!real_path)
    return CURLE_OUT_OF_MEMORY;
	
  new_path = (char*)calloc(4096,sizeof(char));
	strcat(new_path, host);
	new_path_len += strlen(host);

	if (strcmp(real_path, "/") != 0) {
		strcat(new_path, "/");
		new_path_len++;
  	strcat(new_path, real_path);
		new_path_len += strlen(real_path);
	}
  ...
  real_path = new_path;
	...
	new_path = local_handler->get_absolute_path(real_path);
  ...
  */
  ...
  if(!data->set.upload && (fd == -1)) {
    ...
    custom_done(conn, CURLE_FILE_COULDNT_READ_FILE, FALSE);
...
static CURLcode custom_upload(struct connectdata *conn)
{
  struct FILEPROTO *file = conn->data->state.proto.file;
 ...
 * function.
 */
static CURLcode custom_do(struct connectdata *conn, bool *done)
{
