...
builddir = File.expand_path(File.join(topdir,'build',OS))

rootdir = File.join(builddir, NAME + (OS=='osx' ? '.app':'' ))
...
FileUtils.mkdir_p [builddir,appdir,installdir, resourcesdir]
if OS=='osx'
...
Zip::ZipFile.open(runtime_zip, Zip::ZipFile::CREATE) do |zipfile|
  ...
  Dir["#{builddir}/**"].each do |f|
    name = f.gsub(builddir+'/','')
    zipfile.add name,f if installer?(name)
  end
  Dir["#{builddir}/*.nib"].each do |f|
    name = f.gsub(builddir+'/','')
...
FileUtils.cp File.join(builddir,'kinstall'+ext), File.join(installdir,'kinstall'+ext)
  ...
  Zip::ZipFile.open(plugin_zip, Zip::ZipFile::CREATE) do |zipfile|
    Dir["#{builddir}/**"].each do |f|
      ...
      zipfile.add name,f
		...
		Dir["#{builddir}/modules/*"].each do |f|
			modname = f.gsub(builddir+'/modules/', '')
			modname_nodot = modname.gsub(/\./, '')
			next unless modname_nodot.downcase.index(plugin)
			Dir["#{builddir}/modules/#{modname}/Resources/**/*"].each do |r|
				name = r.gsub(builddir+'/modules/'+modname+'/', '')
				zipfile.add name, r
			end
		end
  end
...
FileUtils.cp File.join(builddir,'kboot'+ext), File.join(bindir,NAME+ext)
...
#copy test app files
FileUtils.cp_r(thisdir+'/resources', appdir)

FileUtils.cp_r(thisdir+'/tiapp.xml', appdir)

if OS == 'osx'
  FileUtils.cp_r(thisdir+'/Info.plist', appdir)
  #FileUtils.cp_r(supportdir+'/titanium.icns', )
end
