...
$titanium.debug "parsing options"
...
#t_dir = File.symlink?(t_dir) ? File.readlink(t_dir) : t_dir
TITANIUM_DIR = File.join(SYSTEMDIR, 'titanium')


if OPTIONS[:debug]
$titanium.debug "SYSTEMDIR=#{SYSTEMDIR}"
$titanium.debug "SCRIPTDIR=#{SCRIPTDIR}"
$titanium.debug "LIB_DIR=#{LIB_DIR}"
$titanium.debug "RELEASE_DIR=#{RELEASE_DIR}"
$titanium.debug "PLUGINS_DIR=#{PLUGINS_DIR}"
end
...
TITANIUM_FILES = { 
  "boot.rb" => "#{TITANIUM_DIR}/lib/boot_titanium.rb",
  "install.rb" => "#{TITANIUM_DIR}/lib/install_titanium.rb",
  "install_web_sdk.rb" => "#{TITANIUM_DIR}/lib/install_web_sdk_titanium.rb"
}
...
Dir["#{SYSTEMDIR}/lib/*.rb"].sort{|a,b| File.basename(a)<=>File.basename(b) }.each do |file|
  next if File.basename(file)=~/^_/ # don't auto-load _ files

  $titanium.debug "Loading library: #{file}" if OPTIONS[:debug]
	if TITANIUM_FILES.has_key?(File.basename(file))
    require File.expand_path(TITANIUM_FILES[File.basename(file)])
  else
...
Dir["#{SYSTEMDIR}/commands/*.rb"].sort{|a,b| File.basename(a)<=>File.basename(b) }.each do |file|
  $titanium.debug "Loading command: #{file}" if OPTIONS[:debug]
  require File.expand_path(file)
end

Dir.chdir File.join(TITANIUM_DIR, "lib")
