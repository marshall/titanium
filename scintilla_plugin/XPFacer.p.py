...
# IDL uses wstring
    ...
    IDL_in_string =     ["in ", 	"wstring",	"NPString"]
...
# #if ARCHITECTURE == 'x86_64'
	"ptr": 			["in ", 	"long long",	"uint32 "],
# #else
	"ptr": 			["in ", 	"long",	"int32 "],
# #endif
	"bool": 		["in ", 	"boolean",	"bool "], 
	"position": 		["in ", 	"long",	"int32 "], 
	"colour": 		["in ", 	"long",	"int32 "], 
	"keymod": 		["in ",		"long",	"int32 "], 
	"void": 		["in ", 	"void",	"void"],
...
def castIfNeeded(name, sourceType, destType):
	if sourceType == "_force_cast_":
		...
		return "reinterpret_cast<" + destType + ">(" + name + ")"
	elif cxxType(sourceType) == "NPString":
		return "reinterpret_cast<" + destType + ">(NPStringToString(" + name + ")"
	...
	for name in f.order:
		...
		if v["FeatureType"] in ["fun", "get", "set"] and \
			...
			if wouldBeFuncDef:	# Setter with getter so no extra line in idl but want definition in implementation
				...
				funcImpl = "/* " + funcDef + " */\n"
			if allTypesKnown:
				funcImpl = funcImpl + cxxType(retType) + " SciMoz::"
