...
namespace ti 
{
	FileStream::FileStream(std::string filename_) : stream(NULL)
	{
		...
		/**
		 * @tiapi(method=True,name=Filesystem.Filestream.writeLine,since=0.4) write line into the file
		...
		/**
		 * @tiapi(method=True,name=Filesystem.Filestream.ready,since=0.4) check to see if file is valid for IO
		 * @tiresult(for=Filesystem.Filestream.ready,type=boolean) returns true if open and valid for IO operations
		 */
		this->SetMethod("ready",&FileStream::Ready);
		/**
		 * @tiapi(method=True,name=Filesystem.Filestream.isOpen,since=0.4) check to see if file has been opened
		 * @tiresult(for=Filesystem.Filestream.isOpen,type=boolean) returns true if file is open
		 */
