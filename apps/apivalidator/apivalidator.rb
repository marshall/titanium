...
builddir = File.join(thisdir,'..','..','build')
outdir = File.join(builddir, OS)
...
#copy test app files
FileUtils.mkdir_p(appdir+"/Resources")

FileUtils.cp_r(thisdir+'/public/.', appdir+"/Resources")
FileUtils.cp(thisdir+'/config/tiapp.xml', appdir)

system File.join(appdir, NAME+ext)
